...
def test_clip_path_and_thumbnail_path_are_namespaced_by_camera(
    storage: LocalClipStorage,
) -> None:
    camera_id = uuid.uuid4()
    clip_id = uuid.uuid4()
    assert storage.clip_path(camera_id, clip_id) == storage.root / str(camera_id) / f"{clip_id}.mp4"
    assert (
        storage.thumbnail_path(camera_id, clip_id)
